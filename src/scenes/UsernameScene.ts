...
export class UsernameScene extends Phaser.Scene {
  ...
  create(): void {
    const pendingState = (
      this.scene.settings.data as { pendingState?: PendingAuthState }
    )?.pendingState;

    this.starfield = new Starfield(this);

    this.add
      .text(240, 280, "Choose your username", {
        fontSize: "22px",
        color: "#ffffff",
    ...
    this.add
      .text(240, 316, "3–20 chars · letters, numbers, underscores", {
        fontSize: "13px",
        color: "#888888",
        fontFamily: "monospace",
      })
  ...
  private async handleConfirm(pendingState?: PendingAuthState): Promise<void> {
    ...
    try {
      ...
      if (
        pendingState?.returnTo === "game-over" &&
        pendingState.score !== undefined
      ) {
        await saveScore(
          pendingState.score,
          pendingState.sessionId ?? "",
          pendingState.sessionToken ?? "",
        );
